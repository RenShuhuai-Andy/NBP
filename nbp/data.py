...
from .video_utils import VideoNorm, load_video_from_path_decord, load_video_from_path_tvio, VideoRandomSquareCrop, Magvit2VideoNorm
...
class DecordVideoDataset(data.Dataset):
    ...
    def __init__(self, data_folder, data_list=None, fps=None, sequence_length=17, train=True, resolution=64, resizecrop=False, tokenizer='magvit2', frm_sampling_strategy='center'):
        """
        Args:
            data_folder: path to the folder with videos. The folder
                should contain a 'train' and a 'test' directory,
        ...
        if tokenizer == 'magvit2':
            self.norm = Magvit2VideoNorm()
        else:
            raise ValueError('tokenizer %s is not implementated' % tokenizer)
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            if video_path.endswith("webm"):
                frames, _ = load_video_from_path_tvio(
                    video_path,
                    frm_sampling_strategy="rand",  # always use rand strategy in training
            ...
            if video_path.endswith("webm"):
                frames, _ = load_video_from_path_tvio(
                    video_path,
                    frm_sampling_strategy=self.frm_sampling_strategy,  # may use center or rand in eval
                    fps = self.fps if self.fps is not None else -1,
                    num_frm=self.sequence_length,
                    height=read_height,
                    width=read_width,
        ...
        if self.tokenizer == 'magvit2':
            tensor_frms = torch.from_numpy(frames) if type(frames) is not torch.Tensor else frames  # [T, H, W, C]
            tensor_frms = torch.stack([self.norm(frame.numpy()) for frame in tensor_frms])  # [T, C, H, W]
            video = tensor_frms.permute(1, 0, 2, 3).float()  # (C, T, H, W)
        else:
            raise ValueError('tokenizer %s is not implementated' % self.tokenizer)
        class_name = get_parent_dir(video_path)
        label = self.class_to_label[class_name]
        text = re.sub('([a-z])([A-Z])', r'\1 \2', class_name).lower()  # split camel case for ucf101
        return {"video": video, "label": label, "text": text, "path": video_path}
            ...
            for dataset_path, train_list, val_list in zip(self.args.data_path, self.args.train_datalist, self.args.val_datalist):
                if "UCF" in dataset_path or 'k400' in train_list or 'k600' in train_list or "sthv2" in train_list or "ucf" in train_list or "moment" in train_list:
                    dataset = DecordVideoDataset(dataset_path, train_list if train else val_list, self.args.fps, self.args.sequence_length, 
                                    train=train, resolution=self.args.resolution, resizecrop=self.args.resizecrop, tokenizer=self.args.tokenizer)
