...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True, tokenizer='magvit2'):
        super().__init__()
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            ...
            if self.tokenizer == 'magvit2':
                ...
                ValueError('tokenizer %s is not implementated' % self.tokenizer)
                ...
                if isinstance(images[k], torch.Tensor):
                    ...
                    if self.clamp:
                        if self.tokenizer == 'magvit2':
                            ...
                            images[k] = images[k].squeeze(2)  # [B, C, H, W]
                        else:
                            ValueError('tokenizer %s is not implementated' % self.tokenizer)
...
class VideoLogger(Callback):
    def __init__(self, batch_frequency, max_videos, clamp=True, increase_log_steps=True, tokenizer='magvit2'):
        super().__init__()
        self.batch_freq = batch_frequency
        ...
        if (self.check_frequency(batch_idx) and  # batch_idx % self.batch_freq == 0
            ...
            for k in videos:
                ...
                if isinstance(videos[k], torch.Tensor):
                    ...
                    if self.clamp:
                        if self.tokenizer == 'magvit2':
                            videos[k] = torch.clamp(videos[k], 0, 1)
                        else:
                            ValueError('tokenizer %s is not implementated' % self.tokenizer)
