...
def read_hadoop_file(file_path):
    try:
...
def load_video_from_path_decord(
    ...
    if not height or not width:
        if video_path.startswith("hdfs://"):
            binary_data = read_hadoop_file(video_path)
            vr = VideoReader(io.BytesIO(binary_data), num_threads=4)
        else:
            vr = VideoReader(uri=video_path)
    else:
        if video_path.startswith("hdfs://"):
            binary_data = read_hadoop_file(video_path)
            vr = VideoReader(io.BytesIO(binary_data), num_threads=4, width=width, height=height)
        else:
            vr = VideoReader(uri=video_path, width=width, height=height)
