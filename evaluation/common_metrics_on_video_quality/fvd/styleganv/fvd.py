...
def get_feats(videos, detector, device, bs=10):
    ...
    with torch.no_grad():
        for i in range((len(videos)-1)//bs + 1):
            stacked_video = torch.stack([preprocess_single(video) for video in videos[i*bs:(i+1)*bs]]).to(device)
            assert stacked_video.device == next(detector.parameters()).device, f"{stacked_video.device} != {next(detector.parameters()).device}"
            feats = np.vstack([feats, detector(stacked_video, **detector_kwargs).detach().cpu().numpy()])
    return feats
...
def preprocess_single(video, resolution=224, sequence_length=None, normalize=True):
    # video: CTHW, [0, 1]
    c, t, h, w = video.shape
